...
export function Leaderboard() {
  ...
  const load=useCallback(async()=>{if(!configured){setState("error");setLoading(false);return;} try {const {data,error}=await createClient().rpc("get_leaderboard"); if(error) throw error; setRows((data??[]) as LeaderboardRow[]);setUpdated(new Date());} catch {setState("error");} finally {setLoading(false);}},[configured]);
  ...
  <div className="shell">{loading?<SkeletonCards count={5}/>:state==="error"&&!ranked.length?<EmptyState title="Rankings unavailable" description="The live database could not be reached. Please try again later."/>:!ranked.length?<EmptyState title="No scores yet" description="Rankings will appear after judges complete both sessions."/>:<>
