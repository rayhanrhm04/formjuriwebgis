...
export function AdminShell({children}:{children:React.ReactNode}) {
  ...
  useEffect(()=>{
    let active=true;
    void fetch("/api/admin/session",{cache:"no-store"}).then(async response=>{if(!response.ok)throw new Error("Unable to connect to the competition database.");return response.json() as Promise<{authenticated:boolean}>}).then(result=>{
      if(active)setAuthenticated(result.authenticated);
  ...
  return <div className="admin-grid"><aside className="admin-sidebar"><Brand/><nav>{nav.map(([label,href,Icon])=><Link key={href} href={href} className={path===href?"active":""}><Icon size={19}/><span>{label}</span></Link>)}</nav><button className="button button-secondary" onClick={logout}><LogOut size={17}/> Sign out</button></aside><div className="admin-content"><header className="admin-mobile-head"><Brand compact/><button aria-label="Sign out" onClick={logout}><LogOut size={18}/></button></header>{children}</div><nav className="admin-bottom">{nav.slice(0,4).map(([label,href,Icon])=><Link key={href} href={href} className={path===href?"active":""}><Icon size={19}/><span>{label}</span></Link>)}</nav></div>;
}
